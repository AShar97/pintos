...
init_thread (struct thread *t, const char *name, int priority)
{
	...
	/*My Implementation*/
	t->orig_priority = t->priority = priority;
    t->donated = false;
    t->blocked = NULL;
    list_init (&t->locks);
...
sort_thread_list (struct list *l)
{
  ...
  list_sort (l, priority_more, NULL);
}
...
{
  if (!curr->donated)
		curr->priority = curr->orig_priority = new_priority;
	else if (forced)
		{
...
thread_yield_head (struct thread *cur)
{
	...
	if (cur != idle_thread)
	{
		 ...
		 * the thread with highest priority in the ready list get run first
		 */
		list_insert_ordered(&ready_list, &cur->elem, priority_more_equal, NULL);
		//list_push_back (&ready_list, &cur->elem);
	}
	cur->status = THREAD_READY;
	schedule ();
...
/* Returns true if thread A->priority is bigger than or equal to thread B->priority,
 * false otherwise.
 */
bool priority_more_equal (const struct list_elem *a_, const struct list_elem *b_, void *aux UNUSED)
{
	ASSERT (a_ != NULL);
	ASSERT (b_ != NULL);
	const struct thread *a = list_entry (a_, struct thread, elem);
